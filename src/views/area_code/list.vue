...
import { listCode, deleteAreaCode } from '../../api/area_code'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push('/area_code/create')
    },
    handleUpdate(row) {
      // alert(row.city)
      this.$router.push(`/area_code/edit/${row.city}`)
    },
    handleDelete(row) {
      this.$confirm('此操作将永久删除', '提示', {
        ...
        // 选确定后，才会执行
        deleteAreaCode(row.city).then(response => {
          this.$notify({
            title: '成功',
